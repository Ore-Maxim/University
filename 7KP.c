...
float getelem(matrix1 *a,int x,int y)
{
	...
	while(cur1!=0)
	{
		...
		if(x1==x) return value1;
		cur1=next1;int cur1=a->arr_M[y];
		if(cur1==0)
		{
...
void setelem(matrix1 *a,int x,int y,float val)
{
	...
	else if(val==0)
	{
		...
		while(cur1!=0)
		{
			...
			if(x1==x)
			{
				if(prev1==0) a->arr_M[y]=next1;
				else a->arr_A[prev1*3+2]=next1;
				arr_remove((int**)(&a->arr_A),&a->arr_A_len,cur1*3,3);
				for(int i=0;i<a->arr_M_len;i++) if(a->arr_M[i]>cur1) a->arr_M[i]--;
				for(int i=2;i<a->arr_A_len;i+=3) if(a->arr_A[i]>cur1) a->arr_A[i]--;
				break;
			}
			prev1=cur1;
			cur1=next1;
...
{
	printf("M: ");
	for(int i=0;i<a->arr_M_len;i++) printf("%d ",a->arr_M[i]);
	printf("\n");
	printf("A: ");
	for(int i=3;i<a->arr_A_len;i+=3) printf("%d %.2f %d ",(int)a->arr_A[i],a->arr_A[i+1],(int)a->arr_A[i+2]);
	printf("\n");
}
void matrix1_printnormal(matrix1 *a)
{
	...
	{
		for(int x=0;x<a->w1;x++) printf("%.2f\t",getelem(a,x,y));
		printf("\n");
	}
}
...
void func1(matrix1 *a)
{
	...
	for(int y=0;y<a->h1;y++)
	{
		for(int x=0;x<a->w1;x++)
		{
			...
			if(is > 0) setelem(a, x, y, n / max);
			else setelem(a, x, y, n);
		}
	}
}
